...
class RuotaRomboMatrice {
    ...
    int[][] ruotaRomboMatrice(int[][] t, int k) {
        ...
        for (int i = 0; i < k; i++) t[c + i][c + k - i] = t[c + i + 1][c + k - i - 1]; // destra
        for (int i = 0; i < k; i++) t[c + k - i][c - i] = t[c + k - i - 1][c - i - 1]; // sotto
        for (int i = 0; i < k; i++) t[c - i][c - k + i] = t[c - i - 1][c - k + i + 1]; // sinistra
        for (int i = 0; i < k; i++) t[c - k + i][c + i] = t[c - k + i + 1][c + i + 1]; // sopra
